...
public class ChatClientRpcReflectionWorker implements Runnable, IChatObserver {
    ...
    private Response handleGET_OFFLINE_FRIENDS(Request request){
        System.out.println("GetOFFLIMEFriends Request ...");
        UserDTO udto=(UserDTO)request.data();
        User user=DTOUtils.getFromDTO(udto);
        try {
            User[] friends=server.getOfflineFriends(user);
            UserDTO[] frDTO=DTOUtils.getDTO(friends);
            return new Response.Builder().type(ResponseType.GET_OFFLINE_FRIENDS).data(frDTO).build();
        } catch (ChatException e) {
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
        }
    }
